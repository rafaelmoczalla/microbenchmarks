...
if [ ! -z "/spark/applications/SparkJob-0.1.0.jar" ]; then
    ...
    /spark/bin/spark-submit         --class org.moczalla.joins.distributedjoin.SparkMain         --master spark://node-main:7077         --packages "org.apache.spark:spark-streaming-kafka-0-10_2.12:3.3.0"                  /spark/applications/SparkJob-0.1.0.jar
else
    if [ ! -z "${SPARK_APP_PYTHON_LOCATION}" ]; then
        ...
        echo "Passing arguments "
        PYSPARK_PYTHON=python3  /spark/bin/spark-submit             --master spark://node-main:7077             --packages "org.apache.spark:spark-streaming-kafka-0-10_2.12:3.3.0"                          ${SPARK_APP_PYTHON_LOCATION}
    else
